...
def clean_cartopy_data_dir():
    ...
    if os.path.exists(cartopy_data_dir) and len(os.listdir(cartopy_data_dir)) > 0:
        print(
            "Cartopy data directory is already populated with shapefile data. Cleaning now."
        )
...
if __name__ == "__main__":
    ...
    wrf_input_path = os.path.join(
        os.path.dirname(test_file_path), "wrfout_data", "wrfout_d01_2021-05-13_00_00_00"
    )
    output_plot_dir = os.path.join(home, "Documents", "test_output_images")
    cartopy_data_dir = os.path.abspath(cartopy.config["data_dir"])
    wrfplot_path = os.path.join(
        os.path.dirname(test_file_path), "..", "wrfplot", "wrfplot.py"
    )
