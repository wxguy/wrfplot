...
class Wrfplot(object):
    ...
    def __init__(self, input_path, output_path=None, variables=[], ulevels=None, dpi=150):
    ...
    def read_data(self, var):
        ...
        if self.var in cape_2d:
            self.var_data = getvar(self.nc_fh, 'cape_2d', ALL_TIMES)[cape_2d.index(self.var)]
        elif self.var in cloudfrac:
            self.var_data = getvar(self.nc_fh, 'cloudfrac', ALL_TIMES)[cloudfrac.index(self.var)]
        elif self.var in cape_3d:
            self.var_data = getvar(self.nc_fh, 'cape_3d', ALL_TIMES)[cape_3d.index(self.var)]
            ...
            self.var_data = getvar(self.nc_fh, 'RAINC', ALL_TIMES) + getvar(self.nc_fh, 'RAINNC', ALL_TIMES)
        elif self.var == "ppn":
            self.proj = get_cartopy(getvar(self.nc_fh, 'T2'))
    ...
    def to_datetime(self, date):
        ...
        """
        timestamp = ((date.Time.values - np.datetime64('1970-01-01T00:00:00'))
                     / np.timedelta64(1, 's'))
    ...
    def plot_upper(self):
        ...
        for index in pbar_var:
            ...
            for self.ulevel in pbar_lvl:
                ...
                self.plot_wrf_data(_time, index, self.ulevel)
    ...
    def plot_wrf_data(self, _time, index, data_plot=None, level=None):
        ...
        try:
            if self.U is not None and self.V is not None:
                plot_map = plot.MakePlot(var_name=self.var, data=self.convert_unit(data_plot),
                                         u_kt=self.U[index], v_kt=self.V[index], lons=self.lons, lats=self.lats,
                                         u_level=self.ulevel, run_time=self.cycle, fcst_hr=_time, proj=self.proj,
                                         clevels=self.clevels, output_dir=self.output, dpi=self.dpi, config_file=self.config)
            else:
                plot_map = plot.MakePlot(var_name=self.var, data=self.convert_unit(data_plot),
                                         lons=self.lons, lats=self.lats, u_level=self.ulevel, run_time=self.cycle,
                                         fcst_hr=_time, proj=self.proj, clevels=self.clevels, output_dir=self.output,
...
def _praser():
    ...
    prog_name = 'Command line application to plot static WRF model prognostic products...'
    parser = argparse.ArgumentParser(description=prog_name,
                                     epilog='\u00a9 J Sundar, wrf.guy@gmail.com, 2022')
    parser.add_argument('--list-vars', action='store_true',
                        help="List variables supported by wrfplot.")
    parser.add_argument('--input', metavar='<input_file>', type=arguments.file_path,
                        help="Mandatory path to WRF generated netCDF.")
    parser.add_argument('--output', metavar='<output_dir>', type=arguments.dir_path, default=False,
                        help="Path to output directory for saving images.")
    parser.add_argument('--vars', metavar='<variable(s)>', type=arguments.validate_vars,
    ...
    parser.add_argument('--dpi', metavar='<value>', type=int,
...
def main():
    ...
    if args.list_vars:
        ...
        sys.exit("You must provide path to WRF model output file using '--input' option, output directory for saving image files using '--output' option and must provide at least one variable name using '--var' option to process.\nTypical usage will be \"wrfplot --input filename' --output 'path/to/output/dir' --var 'slp'\"")
    elif all([args.input, args.vars, args.output]):
        file = fileio.FileIO(args.input)
        if file.is_wrf():
            wrfplt = Wrfplot(input_path=args.input, output_path=args.output, dpi=args.dpi)
