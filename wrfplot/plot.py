...
class MakePlot(object):
    ...
    def __init__(self, var_name=None, data=None, u_kt=None, v_kt=None, lons=None, lats=None, run_time=None, fcst_hr=None, u_level=None, proj=None, cmap=None,
                 clevels=None, fig_format='png', output_dir=None, dpi=150, config_file=None):
    ...
    def create_fig(self):
        ...
        if self.ax is not None or self.fig is None:
            self.fig, self.ax = plt.subplots(figsize=(12, 8), subplot_kw=dict(projection=self.proj), frameon=True, num=1, clear=True)
    ...
    def add_shp_features(self):
        ...
        wld_shp_features = ShapelyFeature(Reader(wld_shp_f).geometries(),
    ...
    def add_grids(self):
        ...
        """
        self.grd_lns = self.ax.gridlines(draw_labels=True, color='gray', alpha=0.5, linestyle='--')
    ...
    def plot_title(self):
        ...
        if self.ulevel is not None:
            title_text = title + ' (' + unit + ') at ' + str(self.ulevel) +\
                         ' hPa\nModel Run Hr : ' + self.cycle + ' UTC  |  Fcst Hr : ' + self.fcst_hr + ' UTC'
        else:
            title_text = title + ' (' + unit + ')\nModel Run Hr : ' + self.cycle + ' UTC  |  Fcst Hr : ' + self.fcst_hr + ' UTC'
        self.ax.set_title(title_text, fontsize=12, weight='semibold',
                          style='oblique', stretch='normal', family='serif')
    ...
    def plot_cbar(self):
        ...
        F.add_axes(cax)
        bar = plt.colorbar(self.cf, cax=cax, orientation='vertical', extend=self.c_bar_extend)
        unit = self.config.get(self.var_name, "unit").replace('"', "")
    ...
    def plot_data(self):
        ...
        if self.U is not None and self.V is not None:
            ...
            self.ax.barbs(to_np(self.lons)[::thin, ::thin], to_np(self.lats)[::thin, ::thin], to_np(self.U)[::thin, ::thin],
                          to_np(self.V)[::thin, ::thin], transform=ccrs.PlateCarree(), length=5.5, sizes={'spacing': 0.2},
                          pivot='middle')
    ...
    def save_fig(self):
        ...
        plt.savefig(os.path.join(self.output_dir, filename), bbox_inches='tight', dpi=self.dpi, frameon=True)
